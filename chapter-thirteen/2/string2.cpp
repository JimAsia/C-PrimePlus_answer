...
{
    int total_len = st.len + st2.len;
    char* temp = new char[total_len + 1];
    strcpy_s(temp, st.len + 1, st.str);
    strcpy_s(temp + st.len, st.len + st2.len + 1, st2.str);
    temp[total_len] = '\0';
    return (String)temp;
}
...
{
    for (int i = 0; i <= len + 1; i++)    // i <= this->len + 1
    {
        str[i] = tolower(str[i]);
    }
}
...
void String::stringup()
{
    ...
    {
        str[i] = toupper(str[i]);             // this->str[i] = toupper(this->str[i]);
    }
}
